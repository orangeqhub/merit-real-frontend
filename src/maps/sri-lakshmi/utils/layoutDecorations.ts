...
// road-corridor-50 (the 40ft road beside plots 202-212/213-223) and the
// intersection filler road-added-212-213 sit next to two other near-square
// pieces (road-added-213, road-corridor-88). Those near-square shapes read
// as "vertical" to the automatic strip clustering in roadCenterLines(), so
// it merges all four into one group and averages their mismatched lateral
// positions into a single centerline that drifts off the real road and
// only covers part of the run. Give this stretch explicit, correctly
// centered vertical lines instead, matching the reference (a single
// straight dashed road, no cross-street, running the full length).
const MANUAL_CENTER_LINE_IDS = ["road-corridor-50", "road-added-212-213"];
// road-corridor-160 (below plot 242) sits between road-corridor-159 (below
// plot 241) and the added infill road-added-242-openspace (extending the
// road east under the Ac 0.53 open space, matching the reference). The
// automatic clustering merges all three into one horizontal group but
// drops the middle segment's own coverage, so give it an explicit line too.
const MANUAL_CENTER_LINE_IDS_HORIZONTAL = ["road-corridor-160"];
// World-space bounding boxes covering each mis-clustered group, used to
// drop the automatic (wrong) centerline it produces so it doesn't render a
// second, offset dashed line alongside the manual ones above.
const SUPPRESS_AUTO_CENTER_LINE_BOUNDS = [
  { minX: 22200, maxX: 23350, minY: 8950, maxY: 15100 },
  { minX: 24350, maxX: 25175, minY: 21600, maxY: 22000 },
];

function boundsOf(road: RegionPolygon) {
  const xs = road.polygon.map((p) => p.x);
  const ys = road.polygon.map((p) => p.y);
  return {
    minX: Math.min(...xs),
    maxX: Math.max(...xs),
    minY: Math.min(...ys),
...
function manualCenterLines(): Point[][] {
  const data = regionDataOf();
  const lines: Point[][] = [];
  for (const id of MANUAL_CENTER_LINE_IDS) {
    const road = data.roads.find((r) => (r as { id?: string }).id === id) as
      | (RegionPolygon & { id?: string })
      | undefined;
    if (!road) continue;
    const { minX, maxX, minY, maxY } = boundsOf(road);
  ...
  for (const id of MANUAL_CENTER_LINE_IDS_HORIZONTAL) {
    const road = data.roads.find((r) => (r as { id?: string }).id === id) as
      | (RegionPolygon & { id?: string })
      | undefined;
    if (!road) continue;
...
function isWithinSuppressBounds(points: Point[]): boolean {
  return SUPPRESS_AUTO_CENTER_LINE_BOUNDS.some((b) =>
    points.every(
