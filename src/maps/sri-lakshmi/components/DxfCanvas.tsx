...
import RoadLegend from "./Viewer/RoadLegend";
import ReferenceComparePanel, { type CompareLayers } from "./Viewer/ReferenceComparePanel";
...
import LandmarkLabelsLayer from "./Layers/LandmarkLabelsLayer";
import PlotLayer from "./Layers/PlotLayer";
import ImageUnderlayLayer from "./Layers/ImageUnderlayLayer";
import LayoutBoundaryLayer from "./Layers/LayoutBoundaryLayer";
import BoundaryLandscapingLayer from "./Layers/BoundaryLandscapingLayer";
...
const DxfCanvas: React.FC<Props> = ({
  ...
  // Dev-only reference alignment tool (?compare=1). Never active in
  // production; lets you fade the reference photo against the live SVG to
  // verify geometry visually instead of by eye against a static screenshot.
  const compareMode = (() => {
    try {
  ...
  // Defaults favor a strongly-visible SVG with a faint reference underneath —
  // a 50/50 blend of two similarly-toned fills is visually ambiguous about
  // whether something is "present but faint" vs "missing", which produces
  // false read outs. Push both sliders toward 50/50 only when checking for
  // *positional* alignment; use low reference / high SVG to check *presence*.
  const [referenceOpacity, setReferenceOpacity] = useState(0.4);
  const [svgOpacity, setSvgOpacity] = useState(0.85);
  const [compareLayers, setCompareLayers] = useState<CompareLayers>({
    boundary: true,
    roads: true,
    plots: true,
    labels: true,
  ...
  // Reports this layout's own verified plot list to the parent frontend
  // (used for the "All plots (N)" board, and to build a board plot list
  // that exactly matches the map's own real plot set instead of whatever
  // subset happens to be synced in the booking backend). Computed from the
  // same deduplicated getGenuinePlots() source used for rendering/search/
  // hover — never a hardcoded number, and never the raw
  // plotNumberMapping.json length (which still includes known
  // duplicate-polygon artifacts).
  useEffect(() => {
    try {
      if (window.parent && window.parent !== window) {
        const genuinePlots = getPlotNumberMapping() as unknown as PlotNumberEntry[];
        const plotNumbers = [
          ...new Set(genuinePlots.map((p) => Number(p.plotNumber))),
        ].sort((a, b) => a - b);
        window.parent.postMessage(
          { type: "merit-map-plot-count", count: plotNumbers.length, plotNumbers },
          "*"
        );
      }
    } catch {
  ...
  // DXF/CAD coordinates are Y-up (larger Y = higher on the drawing), so
  // screen-space rendering needs to flip Y. This layout's data source is an
  // image (plot polygons digitized directly from pixel coordinates), which
  // is natively Y-down (larger Y = further down the photo, same as screen
  // space) — flipping it would render the whole map upside-down relative to
  // the reference photo even though every polygon's position relative to
  // its neighbors stays internally consistent (which is why this went
  // unnoticed through extensive hover/click/identity verification: the
  // image underlay in ?edit=1 goes through this same transform, so it was
  // flipped right along with the plots and still "matched").
  const flipY = layout.dataSource === "dxf";

  const convert = useMemo(
    () => (p: Point) => ({
      x: (p.x - layoutBounds.minX) * fitScale + offsetX,
      y: flipY
        ? height - ((p.y - layoutBounds.minY) * fitScale + offsetY)
        : (p.y - layoutBounds.minY) * fitScale + offsetY,
    }),
    [layoutBounds, fitScale, offsetX, offsetY, height, flipY]
  );
  ...
  const toWorld = useMemo(
    () => (p: Point) => ({
      ...
      y: layoutBounds.minY + (flipY ? height - p.y - offsetY : p.y - offsetY) / fitScale,
    }),
    [layoutBounds, fitScale, offsetX, offsetY, height, flipY]
  );
  ...
  const [hoveredNumberPlotId, setHoveredNumberPlotId] = useState<string | null>(null);
  ...
  const handlePlotHover = (
    ...
    setHoveredNumberPlotId(plot ? plot.id : null);
  };
  ...
  return (
    ...
    >
      ...
      {!compareMode && !editMode && <RoadLegend />}
      ...
      >
        ...
        >
          {/* Reference photo: digitizing/comparison aid ONLY (?edit=1 or
              ?compare=1). The production map below this line is pure SVG
              vector geometry — every road and plot is an independently
              rendered, hoverable/clickable element, not a picture. */}

          {(editMode || compareMode) && layout.imageUnderlay && (
            <ImageUnderlayLayer
              convert={convert}
              scale={fitScale}
              box={layout.imageUnderlay.box}
              src={layout.imageUnderlay.src}
              opacity={compareMode ? referenceOpacity : 1}
            />
          )}

          <g opacity={compareMode ? svgOpacity : 1}>
            {/* Layout boundary: ground plane behind everything, traced from
                the union of digitized plot/road geometry (see layouts/index.ts
                LayoutSpec.boundary doc comment). */}

            {(!compareMode || compareLayers.boundary) && (
              <>
                <BoundaryLandscapingLayer
                  convert={convert}
                  scale={fitScale}
                  boundary={layout.landscapingBoundary}
                />
                <LayoutBoundaryLayer
                  convert={convert}
            ...
            {/* Roads layer (main + internal corridors, at the back) */}

            {(!compareMode || compareLayers.roads) && (
              <>
                <RoadLayer
                  convert={convert}
                ...
                {/* Road width labels ("40 FEET PRO. WIDE ROAD" etc.), read
                    off the reference drawing — reference-derived text, not
                    official survey annotation. */}
                <RegionLayer
                  convert={convert}
                  scale={fitScale}
                  zoom={zoom}
            ...
            {/* Region fills: open spaces / utilities / any raw region plots
                (above roads, below the numbered plot layer) */}
            ...
            {/* Optional amenities/features: landscaping */}

            <TreesLayer convert={convert} scale={fitScale} zoom={zoom} />
            <StreetLightsLayer convert={convert} scale={fitScale} zoom={zoom} />

            {/* Plot polygons: one independent <polygon> per plot, data-driven
                from plotNumberMapping.json (see getPlotNumberMapping()). Every
                plot is its own hoverable/clickable/selectable SVG element. */}

            {(!compareMode || compareLayers.plots) && (
              <PlotNumberLayer
                plots={numberPlots}
                convert={convert}
                scale={fitScale}
                ...
                hoveredId={hoveredNumberPlotId}
                fillColor={fillColor}
                onPlotClick={handleNumberPlotClick}
                onPlotHover={handlePlotHover}
                showLabels={!compareMode || compareLayers.labels}
              />
            )}

            {layout.phaseBoundaries && <PhaseBoundaryLayer convert={convert} />}

            {(!compareMode || compareLayers.labels) && (
              <LandmarkLabelsLayer convert={convert} scale={fitScale} zoom={zoom} />
            )}
          </g>
      ...
      {compareMode && (
        <ReferenceComparePanel
          ...
          onSvgOpacityChange={setSvgOpacity}
          layers={compareLayers}
          onLayersChange={setCompareLayers}
        />
      )}
      ...
      <PropertyPopup
        ...
        onBook={(plot) => {
          ...
          // scripts/seedMapPlots.js ("sl-" + this plot's own source id) --
          // this project's raw ids ("p-107") are not globally unique across
          // layouts and would resolve to no plot (or the wrong one) on the
          ...
          window.location.href = `${appBase}/book-plot/${encodeURIComponent(bookingExternalId)}`;
        }}
      />
    </div>
  );
