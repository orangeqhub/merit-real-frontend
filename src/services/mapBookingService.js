...
export const mapBookingService = {
  ...
  async importSheet({ phase, rows, phase1, phase2, layout } = {}) {
    if (Array.isArray(phase1) && Array.isArray(phase2)) {
      return api('/map/plots/import', {
        method: 'POST',
        token: token(),
        body: { phase1, phase2, layout: layout || undefined },
      });
    }
    return api('/map/plots/import', {
      method: 'POST',
      token: token(),
      body: { phase, rows, layout: layout || undefined },
    });
  },
};
...
export const SRILAKSHMI_MAP_URL = (
  import.meta.env.VITE_SRILAKSHMI_MAP_URL || 'http://localhost:5175'
).replace(/\/$/, '');

export const DOKIPARRU_MAP_URL = (
  import.meta.env.VITE_DOKIPARRU_MAP_URL || 'http://localhost:5176'
).replace(/\/$/, '');

export const MANJUNADHA_MAP_URL = (
  import.meta.env.VITE_MANJUNADHA_MAP_URL || 'http://localhost:5183'
).replace(/\/$/, '');

export const VINFRA_MAP_URL = (
  import.meta.env.VITE_VINFRA_MAP_URL || 'http://localhost:5177'
).replace(/\/$/, '');

export const MANDIRA_MAP_URL = (
  import.meta.env.VITE_MANDIRA_MAP_URL || 'http://localhost:5184'
).replace(/\/$/, '');
